...
(define (bind-id-to-src shader-id port)
  (let* ((src-code (for/vector ((line (in-lines port))) line))
         (sizes (for/list ((line (in-vector src-code))) (string-length line)))
         (sizes (list->s32vector sizes)))
   (glShaderSource shader-id (vector-length src-code) src-code sizes)))
...
(define (make-shader-program)
  (let ((prog-id (glCreateProgram))
        ;(vsh-id (make-a-shader GL_VERTEX_SHADER "vshader.vsh"))
        (fsh-id (make-a-shader GL_FRAGMENT_SHADER "test.glsl")))
...
(define (make-a-shader shader-type shader-src )
  (let ((sh-id (glCreateShader shader-type)))
