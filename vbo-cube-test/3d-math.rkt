...
(require math/matrix  ffi/vector ffi/unsafe)
(provide identity-mat4 translate lookat perspective matrix->f64vector matrix->f32vector)
...
(define (perspective fov aspect znear zfar)
  ...
  (list->matrix 4 4 (list (/ h aspect) 0.0 0.0 0.0
        0.0 h 0.0 0.0 
...
(define (vector* vec1 vec2)
  (vector(- (* (vector-ref vec1 1) (vector-ref vec2 2))  (* (vector-ref vec1 2) (vector-ref vec2 1)))
         (- (* (vector-ref vec1 2) (vector-ref vec2 0))  (* (vector-ref vec1 0) (vector-ref vec2 2)))
         (- (* (vector-ref vec1 0) (vector-ref vec2 1))  (* (vector-ref vec1 1) (vector-ref vec2 0)))))
...
(define (lookat eye-vec center-vec up-vec)
  (define norm-forward (vector-normalize (vector- center-vec eye-vec))) ;ch
  (define norm-side (vector-normalize (vector* norm-forward (vector-normalize up-vec))))
  (define up (vector* norm-side norm-forward))
  (matrix* (matrix-stack (list 
